...
class WDWT_updater
{
  ...
  public function get_old_params()
  {
    ...
    if (!$this->compare_version($this->version_new, $this->version_old)) {
      ...
      if (substr($current_version, 0, 1) == '2' &&
        ...
        /*upgrade to same pro*/
        $this->options = apply_filters('wdwt_new_options_new_users', $this->options);
      }
      /*save theme version*/
  ...
  {
    $this->options[$param_name] = $val;
    /*fix for old slider delimiter format*/
    if ($param_name == 'slider_head' || $param_name == 'slider_head_href' || $param_name == 'slider_head_title' || $param_name == 'slider_head_desc') {
      $this->options[$param_name] = str_replace(';;;;', '||wd||', $this->options[$param_name]);
    }
